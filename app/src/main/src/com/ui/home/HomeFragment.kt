...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private val viewModel: HomeViewModel by viewModels {
        ...
        HomeViewModelFactory(app.repository, app.profileRepository)
    }
    ...
    private val sourceIdByChipLabel: Map<String, String?> by lazy {
        mapOf(
            allSourcesLabel() to null,
            "CNN" to "cnn",
            "BBC" to "bbc-news",
            "Reuters" to "reuters",
            "AP" to "associated-press",
    ...
    private fun renderSourceChips(items: List<NewsUiModel>) {
        val dynamic = items
            .map { it.sourceName.trim() }
            .filter { it.isNotBlank() }
            .distinct()
            .sorted()

        val merged = linkedSetOf(allSourcesLabel(), "CNN", "BBC", "Reuters", "AP", "TechCrunch")
        merged.addAll(dynamic)

        if (selectedSource !in merged) {
            selectedSource = allSourcesLabel()
        }
        ...
        merged.forEach { source ->
            binding.sourceChipContainer.addView(
                createSourceChip(source, source == selectedSource) {
                    if (selectedSource == source) return@createSourceChip
                    selectedSource = source
                    renderSourceChips(currentItems)
                    val sourceId = sourceIdByChipLabel[source]
                    if (sourceId != null || source == allSourcesLabel()) {
                        val currentFilters = viewModel.currentFilterState()
                        viewModel.applyFilters(currentFilters.copy(sourceId = sourceId))
                    } else {
                        applySourceFilter()
                    }
                }
    ...
    private fun applySourceFilter() {
        val filteredByChip = if (selectedSource == allSourcesLabel()) {
            currentItems
        } else {
            currentItems.filter {
                val source = it.sourceName.trim()
                source.equals(selectedSource, ignoreCase = true) ||
                    (selectedSource == "AP" && source.contains("associated press", ignoreCase = true))
            }
        }

        val visibleItems = filteredByChip.filterNot { hiddenSources.contains(it.sourceName.trim().lowercase()) }
    ...
    private fun navigateToDetail(item: NewsUiModel) {
        ...
        findNavController().navigate(
            ...
            bundleOf(
                ...
                "articleUrl" to item.articleUrl.orEmpty(),
    ...
    private fun allSourcesLabel(): String = getString(R.string.home_all_sources)
