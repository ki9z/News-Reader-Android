...
class ProfileFragment : Fragment(R.layout.fragment_profile) {
    ...
    private val viewModel: ProfileViewModel by viewModels {
        ...
        ProfileViewModelFactory(app.userSettingsRepository, app.appDatabase.userDao())
    }
    ...
    private fun showLoginScreen() {
        ...
        // Add LoginRegisterFragment to container
        if (childFragmentManager.findFragmentById(R.id.loginContainer) == null) {
            childFragmentManager.beginTransaction()
                .replace(R.id.loginContainer, LoginRegisterFragment())
                .commit()
        }
    }
    ...
    fun onLoginSuccess() {
        // This is called from LoginRegisterFragment after successful login
        // The observeLoginStatus will automatically update the UI
    }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.settings.collect { settings ->
                    ...
                    // Show/hide admin row based on user role
                    viewModel.currentUser.collect { user ->
    ...
    private fun setupActions() {
        ...
        binding.rowAccountType.setOnClickListener {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.settings.collect { settings ->
                    ...
                    binding.tvAccountType.text = accountTypeLabel(settings.currentAuthType)
                    
                    // Show/hide admin row based on account type
                    val isAdmin = settings.currentAuthType == "admin"
                    binding.rowAdmin.visibility = if (isAdmin) View.VISIBLE else View.GONE
    ...
    private fun showAccountTypeDialog() {
        ...
        val options = listOf("user" to "Tài khoản người dùng", "admin" to "Tài khoản quản trị")
        val selectedIndex = options.indexOfFirst { it.first == currentAuthType }
        ...
        androidx.appcompat.app.AlertDialog.Builder(requireContext())
            ...
            .setSingleChoiceItems(
                ...
                val selectedAuthType = options[which].first
                viewModel.setAuthType(selectedAuthType)
                toast("Đã chuyển sang tài khoản ${options[which].second}")
                dialog.dismiss()
            }
