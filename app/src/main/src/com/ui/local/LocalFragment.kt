...
class LocalFragment : Fragment(R.layout.fragment_local) {
    ...
    private companion object {
        const val MAX_BUNDLE_TEXT_LENGTH = 8_000
    }
    ...
    private fun navigateToDetail(item: NewsUiModel) {
        ...
        findNavController().navigate(
            ...
            bundleOf(
                "title" to safeBundleText(item.title),
                "description" to safeBundleText(item.description),
                "content" to safeBundleText(item.content),
                "imageUrl" to safeBundleText(item.imageUrl),
                "articleUrl" to safeBundleText(item.articleUrl),
                "sourceName" to safeBundleText(item.sourceName),
                "author" to safeBundleText(item.author),
                "publishedAt" to safeBundleText(item.publishedAt),
                "fromScreen" to "local"
            )
        )
    }

    private fun safeBundleText(value: String?): String {
        return value.orEmpty().trim().take(MAX_BUNDLE_TEXT_LENGTH)
    }
